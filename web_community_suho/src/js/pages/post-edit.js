...
async function handleSubmit(e) {
    ...
    try {
        ...
        if (response.ok) {
            alert("게시글이 수정되었습니다!");
            window.location.replace(`./detail.html?postId=${postId}`);
        } else {
...
function loadPostData() {
    ...
    titleField.value = decodeURIComponent(postTitle);
    contentField.value = decodeURIComponent(postContent);
    updateButtonState();
}
...
document.getElementById("postEditForm").addEventListener("submit", handleSubmit);
